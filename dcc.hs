...
import BaseDijkstra.LLVMBindings.ReadAssembly
import BaseDijkstra.LLVMBindings.NamedFunction
import Data.Int
...
options =
  [ Option "h?" ["help"]                  (NoArg Help)                    "Show this help",
    Option "lj" ["emit-llvm", "emit-ir"]  (NoArg OutputIR)                "Output the compiled LLVM IR to inputfile.ll",
    ...
    Option "b"  ["emit-bitcode"] (NoArg CompileToBitcode) "Emit LLVM bitcode (requires an llvm installation) using `llvm-link` -- outputs to inputfile.llb",
    Option "c"  ["compile", "emit-binary"] (NoArg CompileToBinary) "Emit a system appropriate binary executable (requires an LLVM installation and a linker aliased as `ld`) -- outputs to inputfile",
    Option "r"  ["run-program"]  (NoArg RunProgram)  "Does not emit anything by itself.  Instead, runs the program bitcode on llvm",
    Option "o"  ["output"]    (ReqArg OutputName "FILENAME")  "Set the output file name to OUTPUTNAME instead of inputfile"
  ]

data CompilerOptions = CompilerOptions { verbose :: Bool, emitIR :: Bool, emitST :: Bool, emitTree :: Bool, emitBitcode :: Bool, emitBinary :: Bool, emitTAF :: Bool, runProg :: Bool, outputName :: String }
...
parseOptions argv =
  ...
  in
    case errs of
      [] ->
        if hasHelp actions || length actions < 1
...
parseReturnedFlags fn acts =
  let
    defaultOptions = CompilerOptions { verbose = False, emitIR = False, emitST = False, emitTree = False, emitBitcode = False, emitBinary = False, emitTAF = False, runProg = True, outputName = fst $ break ('.' ==) fn } 
  in
    foldl (\opts flag ->
            case flag of
              OutputIR -> opts { emitIR = True, runProg = False }
              Package pkg -> opts
              OutputSymbolTable -> opts { emitST = True, runProg = False }
              OutputInitialTree -> opts { emitTree = True, runProg = False }
              OutputThreeAddressForm -> opts { emitTAF = True, runProg = False }
              Verbose -> opts { verbose = True }
              CompileToBitcode -> opts { emitBitcode = True, runProg = False }
              CompileToBinary -> opts { emitBinary = True, runProg = False }
              RunProgram -> opts { runProg = True }
...
mainCompile = do
  ...
  mod <- lift . readAssemblyFromString $ finalCode
  when (emitBitcode compilerOpts) $ lift . (writeBitcodeToFile (outputBase ++ ".bc")) $ mod

  -- TODO: compile to binary
  when (emitBinary compilerOpts) $ lift $ putStrLn "Direct output to system-specific binary not yet supported"

  when (runProg compilerOpts) $ do
    lift initializeNativeTarget
    compiledFun <- lift $ compileFunc mod "main"
    lift compiledFun
    return ()
  
  return ()

compileFunc :: Module -> String -> IO (IO Int32)
compileFunc mod fn = do
  funcRef <- getNamedFunctionFromModule mod fn
  --funcs <- getFunctions mod
  --funcs <- getModuleValues mod
  --let (Just funcRef) = castModuleValue =<< lookup fn funcs
  --dumpValue funcRef
  runEngineAccess $ do { addModule mod; generateFunctionFromRef funcRef }
  --runEngineAccess $ do { addModule mod; generateFunction funcRef }
